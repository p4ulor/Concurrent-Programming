...
// comment the next line in order to monitor pool with an I/O bound workload

//#define CPU_BOUND_WORKLOAD
...
#if CPU_BOUND_WORKLOAD
        Console.WriteLine("\n-- Monitor .NET Thread Pool using a CPU-bound workload\n");	
#else
		Console.WriteLine("\n-- Monitor .NET Thread Pool using a I/O-bound workload\n");
#endif
		...
		for (int i = 0; i < ACTION_COUNT; i++) {
			...
			ThreadPool.QueueUserWorkItem((targ) => {
				...
				for (int n = 0; n < REPEAT_FOR; n++) {
					WorkerThreadReport.RegisterWorker();
#if CPU_BOUND_WORKLOAD
                    Thread.SpinWait(5000000);		// CPU-bound workload
#else
