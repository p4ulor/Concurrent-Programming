 ...
 *
 * Read writer lock with Hoare's semantics in .NET based in MonitorEx extension and
 * using specific thread notification
 *
 * Generate executable with: csc .\ReadWriteLockHoareSN.cs ..\utils\MonitorEx.cs
 *
...
public class ReadWriteLockHoareSN {
	...
	public void LockRead()  {
		lock(mlock) {
			...
			// otherwise, enqueue a shared lock request
			RdReqNode rdnode;
	...
	public void LockWrite() {
		lock(mlock) {
			// if the lock isn’t held in shared mode nor for exclusive mode and the writers’ wait
			// queue is empty, grant the exclusive access immediately
	...
	public void UnlockRead() {
		lock(mlock) {
			readers--; // decrement the number of active shared locks
			// if this is the last active shared lock, and there is at least a blocked writer, grant access
			// to the writer that is at front of the queue, if any 
